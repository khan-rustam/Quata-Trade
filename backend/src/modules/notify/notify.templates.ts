...
const TEMPLATE_SOURCES: Record<TemplateName, TemplateSource> = {
  ...
  withdrawal_approved: {
    ...
    body: "Your withdrawal{{#if amountDisplay}} of {{amountDisplay}} USDT{{/if}}{{#if addressPreview}} to {{addressPreview}}{{/if}} has been approved and is being sent to the TRON network. You will be notified once it is confirmed on-chain.",
  },
  withdrawal_rejected: {
    subject: "Withdrawal declined",
    body: "Your withdrawal{{#if amountDisplay}} of {{amountDisplay}} USDT{{/if}}{{#if addressPreview}} to {{addressPreview}}{{/if}} was not approved. The amount has been returned to your available balance. Contact support if you believe this was a mistake.",
  },
  withdrawal_failed: {
    subject: "Withdrawal failed",
    body: "Your withdrawal{{#if amountDisplay}} of {{amountDisplay}} USDT{{/if}}{{#if addressPreview}} to {{addressPreview}}{{/if}} could not be completed and the amount has been returned to your available balance. You can try again from your wallet.",
  },
  withdrawal_confirmed: {
  ...
  internal_transfer_received: {
    subject: "You received USDT",
    body: "You received{{#if amountDisplay}} {{amountDisplay}} USDT{{/if}} from another QuataTrade user. It is available in your wallet now.",
  },
  wallet_created: {
    subject: "Your QuataTrade wallet is ready",
    body: "Your USDT (TRC20) deposit wallet has been created and linked to your account. You can now deposit crypto to start trading.",
  },
  trade_escrow_locked: {
...
const EMAIL_CONTENT: Record<TemplateName, (ctx: Record<string, string>, appUrl: string) => EmailContent> = {
  ...
  withdrawal_approved: (ctx, appUrl) => ({
    preheader: "Your withdrawal was approved",
    heading: "Withdrawal approved",
    paragraphs: ["Your withdrawal has been approved and is being broadcast to the TRON network. We'll email you as soon as it's confirmed on-chain."],
    facts: [...amountFact(ctx), ...addressFact(ctx)],
    button: button(appUrl, "/wallet", "View wallet"),
  }),
  withdrawal_rejected: (ctx, appUrl) => ({
    preheader: "Your withdrawal was declined",
    heading: "Withdrawal declined",
    paragraphs: ["Your withdrawal was not approved, and the amount has been returned to your available balance. If you believe this was a mistake, please contact support."],
    facts: [...amountFact(ctx), ...addressFact(ctx)],
    button: button(appUrl, "/wallet", "View wallet"),
  }),
  withdrawal_failed: (ctx, appUrl) => ({
    preheader: "Your withdrawal could not be completed",
    heading: "Withdrawal failed",
    paragraphs: ["Your withdrawal could not be completed and the amount has been returned to your available balance. You can try again from your wallet."],
    facts: [...amountFact(ctx), ...addressFact(ctx)],
    button: button(appUrl, "/wallet", "View wallet"),
  }),
  ...
  internal_transfer_received: (ctx, appUrl) => ({
    preheader: "You received USDT on QuataTrade",
    heading: "You received USDT",
    paragraphs: ["Another QuataTrade user sent you USDT. It's available in your wallet now."],
    facts: amountFact(ctx),
    button: button(appUrl, "/wallet", "View wallet"),
  }),
  wallet_created: (_ctx, appUrl) => ({
    preheader: "Your QuataTrade wallet is ready",
    heading: "Your wallet is ready",
    paragraphs: ["Your USDT (TRC20) deposit wallet has been created and linked to your account. You can now deposit crypto to start trading."],
    button: button(appUrl, "/wallet/deposit", "Get deposit address"),
  }),
